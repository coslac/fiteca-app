...
function EnhancedTableToolbar(props) {
    ...
    const handleExcluir = async () => {
        try {
            setOpen(false);
            if (numSelected > 1) {
                const res = await axios.delete(`${apiURL}/produtos`, { ids: selected });
                ...
                if (res && res.status === 200) {
                    ...
                    props.handleDeleted(res.data.produtos);
                } else {
                    dispatch(
                        showMessage({
                            message: 'Erro ao excluir produtos!',
                            autoHideDuration: 6000,
                            anchorOrigin: {
                                vertical: 'top',
                                horizontal: 'right',
                ...
                const res = await axios.delete(`${apiURL}/produto/${selected[0]}`);
        ...
        <>
            ...
            >
                ...
                {numSelected > 0 ? (
                    ...
                    <>
                        <Tooltip title="Cadastrar Produto" onClick={() => navigate('/produto')}>
                            <IconButton color='primary'>
                                <AddCircleOutline />
                            </IconButton>
                        </Tooltip>
            ...
            >
                <DialogTitle>{`Tem certeza que deseja excluir ${numSelected > 1 ? 'os produtos' : 'o produto'}?`}</DialogTitle>
                <DialogContent>
                    <DialogContentText id="alert-dialog-slide-description">
                        {numSelected > 1 ? `${numSelected} produtos selecionados. Ao excluir os produtos, você não poderá mais recuperá-los.` : '1 produto selecionado. Ao excluir o produto, você não poderá mais recuperá-lo.'}
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell
                                                    ...
                                                    scope="row"
                                                    padding="none"
                                                    onClick={() => navigate(`/produto/${row?.id}`)}
                                                >
                                                    {row?.artigo}
                                                </TableCell>
                                                <TableCell onClick={() => navigate(`/produto/${row?.id}`)} align="left">{row?.largura}</TableCell>
                                            </TableRow>
                                        );
                                    })}
                                    {emptyRows > 0 && (
